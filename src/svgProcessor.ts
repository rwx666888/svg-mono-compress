...
export class SvgProcessor {
  ...
  private async processMonoFile(filePath: string): Promise<CompressResult> {
    ...
    try {
      ...
      const step1Result = optimize(content, baseConfig);
      ...
      const step3Result = optimize(step2Data, monoConfig);
      ...
      const step4Result = optimize(step3Result.data, step4Config);
   ...
   */
  private stripGradientsAndColorStyles(svgContent: string): string {
    let result = svgContent;

    // 1. 删除渐变 / pattern 元素（含子节点）
    result = result.replace(
      /<(linearGradient|radialGradient|pattern)(\s[^>]*)?>[\s\S]*?<\/\1>/gi,
      ''
    );
    // 自闭合形式
    result = result.replace(
      /<(linearGradient|radialGradient|pattern)(\s[^>]*)?\/>/gi,
      ''
    );
   ...
   */
  private ensureRootCurrentColor(svgContent: string): string {
    // 找到根 svg 开标签（第一个 <svg ...>）
    return svgContent.replace(/^(<svg\b[^>]*?)(\s*\/?>)/, (
      _match: string,
      tagBody: string,
      closing: string
    ) => {
  ...
  private calculateProcessResult(results: CompressResult[]): ProcessResult {
    ...
    const totalCompressedSize = results.reduce(
      (sum, r) => sum + (r.success ? r.compressedSize : r.originalSize),
      0
    );
