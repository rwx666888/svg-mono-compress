...
export function activate(context: vscode.ExtensionContext) {
  ...
  const compressCommand = vscode.commands.registerCommand(
    ...
    async (uri: vscode.Uri, uris?: vscode.Uri[]) => {
      await handleSvgCommand(
        uri,
        uris,
        (paths, onProgress) => svgProcessor.compressSvg(paths, onProgress),
        '压缩 SVG'
      );
    }
  );
  ...
  const monoCompressCommand = vscode.commands.registerCommand(
    ...
    async (uri: vscode.Uri, uris?: vscode.Uri[]) => {
      await handleSvgCommand(
        uri,
        uris,
        (paths, onProgress) => svgProcessor.convertToMono(paths, onProgress),
        '转换为单色 SVG'
      );
    }
  );
...
function writeToOutputChannel(
  ...
  channel.appendLine(separator);
  channel.appendLine(
    `汇总：成功 ${successCount} / 共 ${results.length} 个文件` +
    (failedCount > 0 ? `，失败 ${failedCount} 个` : '')
  );
  channel.appendLine(
    `大小：${formatSize(totalOriginalSize)} → ${formatSize(totalCompressedSize)}` +
